...
// -----------------------------------------------------------
// FIX: Changed from synchronous `test` with `forEach` to 
// asynchronous `test` with `for...of` to avoid Unhandled Rejections.
// -----------------------------------------------------------
test("renders a span for each genre", async () => {
  render(<RouterProvider router={router} />);
  const genres = ["Action", "Adventure", "Fantasy"];
  
  // The 'for...of' loop correctly waits for each await call
